---
layout: post
title: '在Windows环境下安装Jekyll的简单方法'
subtitle: 'powershell是个好东西'
date: 2018-10-15 21:48:12
categories: [技术,建站]
cover: 'https://upload-images.jianshu.io/upload_images/975622-4a521c576cc23ce8.png'
tags: [jekyll,powershell,windows,🚀]
---
...
接触过Ruby语言的朋友肯定知道Ruby的各种优点，可以说Ruby是最接近自然语言的编程语言，简洁、优美、强大，记得大一时候在学校图书馆随便找书看，发现了一本『笨办法学Ruby』，看了几页就迷上了这个语言，随着更多的了解知道了Rails的存在，一个现代Web框架的鼻祖，甚至有人评价：Rails之前没有Web开发。Rails的出现改变了Web开发一贯的原始方式，后来的许多语言和Web框架都或多或少在致敬（cao xi）Rails。而Rails就是由DHH使用Ruby语言开发的。无奈随着深入学习，发现国内资料较少，放弃了对Ruby的深入了解，长时间过去了，现在连语法都忘得差不多了 :-D。虽然Ruby优点很多，但是这门语言对使用Windows系统的开发者可以说是及其不友好，安装相关库或运行某框架需要折腾很多东西，学习Ruby平台选择排序为：Mac > Linux > Windows，这也是很多人放弃学习Ruby的原因之一，毕竟不是人人都有Mac（qiong），Linux对小白来说又比较麻烦，而Ruby开发在Windows平台几乎寸步难行。

Jekyll是用Ruby开发的静态站点生成器，一般用来和Github Pages服务配合搭建自己的个人Blog或项目主页，但是并不是每个人都想使用Github Pages服务，这时候就需要安装Jekyll手动生成静态站点文件，Mac和Linux系统安装使用Jekyll还是比较简单的，由于Ruby的Windows不友好，想在Windows系统安装使用Jekyll就比较麻烦了，很多文章给出了N多在Windows上安装Jekyll的步骤，即麻烦又容易让人迷惑。

这里介绍一个在Windows平台安装Jekyll的方法，只需要几个步骤就能完美安装Jekyll并使用。

# 1. 安装Chocolatey

**1.** 改变powershell脚本执行级别

首先用管理员命令打开cmd，然后键入`powershell`并回车进入powershell交互环境，执行`Set-ExecutionPolicy bypass`，意思是不需要签名既可运行脚本。

**2.** 这一步很简单，只需要保存并运行一个powershell脚本即可，[点我下载](https://chocolatey.org/install.ps1)。

*如果在浏览器直接显示内容，可以直接全选复制然后本地新建一个`install.ps1`文件，用编辑器打开粘贴并保存。然后把`install.ps1`放到powershell中执行，或者右键使用powershell运行。（这里的install.ps1既Chocolatey）。可能部分地区由于GFW导致该网页无法打开，文末有这个脚本代码，你可以直接Ctrl+C，Ctrl+V，Ctrl+S。:-D*

**3.** 在cmd执行`SET "PATH=%PATH%;%ALLUSERSPROFILE%\chocolatey\bin"`

**其实也可以不改变powershell执行级别，甚至不用下载保存`install.ps1`文件，对Powershell了解比较深或者用过Empire/Powersploit/Nishang/Metasploit等渗透测试框架的童鞋肯定知道，这里就不多说了。**

# 2. 安装 Ruby

cmd执行`choco install ruby -y`

# 3. 安装Bundler

打开C盘 => 进入tools文件夹 => 进入rubyxx文件夹 => 进入bin文件夹 => 在文件夹空白处同时按shift和鼠标右键进入cmd => 执行`.\gem install bundler`

# 4. 安装Jekyll

在上个cmd窗口继续执行`.\gem install jekyll`，等待执行结束，继续执行`.\gem install jekyll bundler`
...
jekyll serve
```
上述命令一行一个，挨个运行，Jekyll会在`localhost:4000`运行服务，这个网页就是一个Jekyll测试站点。

# 参考链接

- [在 windows 安装 Jekyll](https://lindexi.github.io/lindexi/post/%E5%9C%A8-windows-%E5%AE%89%E8%A3%85-Jekyll.html#安装jekyll)
- 测试
